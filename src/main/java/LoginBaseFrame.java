...
public abstract class LoginBaseFrame extends JFrame {
    private static final String PASSWORD = "12345";

    public LoginBaseFrame(String frameTitle) {
        super(frameTitle);
        setFont(new Font("MinionPro", Font.BOLD, 20));
        setDefaultCloseOperation(JFrame.HIDE_ON_CLOSE);
               
        ImageIcon icon = new ImageIcon(getClass().getResource(getIconPath()));
    ...
    // Method to be impemented by the subclasses for loading a defferent icon for each frame
    protected abstract String getIconPath();

    // Method to be overridden by subclasses. Defines what happens on successful Login.
